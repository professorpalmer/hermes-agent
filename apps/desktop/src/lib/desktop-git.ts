...
const remoteGit: GitBridge = {
  // Source-control operations (status/stage/commit/branch/stash/…) are
  // Electron-local: they shell out to `git` in the working tree. A remote
  // gateway has no local checkout, so these degrade to a clear "unavailable"
  // result rather than silently no-op. The desktop Source Control panel is
  // gated on the Electron bridge (store/git.ts reads window.hermesDesktop.git
  // directly), so these stubs only guard the remote-fs fallback path.
  status: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  diff: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  stage: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  unstage: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  discard: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  commit: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  push: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  pull: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  fetch: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  branches: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  checkout: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  createBranch: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  deleteBranch: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  log: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  commitDiff: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  stashList: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  stashPush: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  stashAction: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  applyHunk: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
  revertEdit: async () => ({ ok: false, error: 'git-unavailable-on-remote' }),
